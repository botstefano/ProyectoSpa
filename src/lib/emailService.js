/**
 * Servicio de email con fallback inteligente
 * Intenta Edge Function primero, si falla usa método directo con fallback
 */
...
export async function enviarFormularioEnriquecimiento(emailCliente, nombreCliente, tokenEnriquecimiento, origen) {
  ...
  // Si no hay API key, proporcionar link manual inmediatamente
  if (!resendApiKey) {
    logger.warn('emailService', 'RESEND_API_KEY no configurada, modo link manual')
    return { 
      success: false, 
      modo: 'link_manual',
      mensaje: 'Email no configurado, pero el formulario está disponible',
  ...
  try {
    ...
    // Fallback: proporcionar link manual para que el sistema siga funcionando
    return { 
      success: false, 
      modo: 'link_manual',
...
export async function enviarRecordatorioEnriquecimiento(emailCliente, nombreCliente, tokenEnriquecimiento, origen) {
  ...
  if (!resendApiKey) {
    return { 
      success: false, 
      modo: 'link_manual',
      mensaje: 'Recordatorio no enviado, pero el formulario está disponible',
      linkManual: `${origen}/enriquecimiento/${tokenEnriquecimiento}`
    }
  }
